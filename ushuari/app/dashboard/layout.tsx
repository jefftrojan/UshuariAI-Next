...
export default function DashboardLayout({
  ...
  useEffect(() => {
    const checkAccess = async () => {
      ...
      const hasAccess = await ensureCorrectRoleAccess(router, ["user"]);
      setIsLoading(false);
    };
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex">
      {/* Sidebar */}
      <aside
        className={`bg-indigo-800 text-white w-64 fixed h-full transition-all duration-300 ${
          isSidebarOpen ? "left-0" : "-left-64"
        }`}
      >
        <div className="p-4 border-b border-indigo-700">
          <h2 className="text-xl font-semibold">Ushuari</h2>
          <p className="text-indigo-300 text-sm">User Dashboard</p>
        </div>
        ...
        <div className="absolute bottom-0 w-full p-4 border-t border-indigo-700">
          <button
            ...
            className="w-full text-left py-2.5 px-4 rounded transition duration-200 hover:bg-indigo-700"
          >
            Logout
          </button>
        </div>
